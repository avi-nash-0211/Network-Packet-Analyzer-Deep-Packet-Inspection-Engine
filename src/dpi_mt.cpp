...
private:
    ...
    void classifyFlow(
        ...
        // DNS
        if (
            pkt.tuple.dst_port == 53 ||
            pkt.tuple.src_port == 53
        ) {
        ...
        // Port-based fallback
        // Don't mark as classified so SNI can be detected later.
...
private:
    ...
    void run() {
        while (running_) {
            ...
            size_t fp_idx =
                hasher(pkt_opt->tuple) %
                num_fps_;

            fps_[fp_idx]->queue().push(
                std::move(*pkt_opt)
            );
...
public:
    ...
    DPIEngine(const Config& cfg)
        ...
        // ASCII-only banner for Windows compatibility
        std::cout << "\n";
    ...
    bool process(
        ...
        std::thread output_thread(
            [&]() {
                ...
                while (
                    output_running ||
                    output_queue_.size() > 0
                ) {

                    auto pkt_opt =
                        output_queue_.pop(50);
                    ...
                    output.write(
                        reinterpret_cast<const char*>(&phdr),
                        sizeof(phdr)
                    );
